...
export class BaseOAuthService {
    ...
    public async findUserByCode(code:string):Promise<TypeUserInfo>{
        ...
        if (!tokensRequest.ok) {
          ...
          const tokenResponse = await tokensRequest.json();
          
          if (!tokenResponse.access_token) {
            throw new BadRequestException(
              `Нет токенов в ${this.options.access_url}. Убедитесь, что код авторизации действителен.`
            );
          }
        ...
        if(!userRequest.ok){
            ...
            throw new UnauthorizedException(`не удалось получить пользователя ${this.options.profile_url}: ${userErrorResponse}`)
        }

        const user = await userRequest.json()
        // извлекавем данные о пользователе
